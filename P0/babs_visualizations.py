...
def filter_data(data, condition):
    ...
    if field not in data.columns.values:
        raise Exception(
            "'{}' is not a feature of the dataframe. Did you spell something wrong?".format(field))
    ...
    if op == ">":
        ...
        raise Exception(
            "Invalid comparison operator. Only >, <, >=, <=, ==, != allowed.")
...
def usage_stats(data, filters=[], verbose=True):
    ...
    if verbose:
        if filters:
            print('There are {:d} data points ({:.2f}%) matching the filter criteria.'.format(
                n_data, 100. * n_data / n_data_all))
        else:
            print('There are {:d} data points in the dataset.'.format(n_data))

        print('The average duration of trips is {:.2f} minutes.'.format(
            duration_mean))
        print('The median trip duration is {:.2f} minutes.'.format(
            duration_qtiles[1]))
        print('25% of trips are shorter than {:.2f} minutes.'.format(
            duration_qtiles[0]))
        print('25% of trips are longer than {:.2f} minutes.'.format(
            duration_qtiles[2]))
...
def usage_plot(data, key='', filters=[], **kwargs):
    ...
    if not key:
        raise Exception(
            "No key has been provided. Make sure you provide a variable on which to plot the data.")
    ...
    if isinstance(data[key][0], str):  # Categorical features
        ...
        for i in range(n_levels):
            trips_bar = plt.bar(i - bar_width / 2,
                                data.loc[i]['count'], width=bar_width)
        ...
        if kwargs and 'n_bins' in kwargs and 'bin_width' in kwargs:
            raise Exception(
                "Arguments 'n_bins' and 'bin_width' cannot be used simultaneously.")
        ...
        if kwargs and 'boundary' in kwargs:
            bound_factor = np.floor(
                (min_value - kwargs['boundary']) / bin_width)
